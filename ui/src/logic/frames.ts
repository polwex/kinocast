...
export function extractOpenGraph(s: string, embedURL: string): OGRes {
  ...
  const tags: HTMLMetaElement[] = Array.from(metaTags)
    .map((meta) => {
...
export function findFrameTags(
  ...
  for (let meta of metaTags) {
    ...
    else if (name === "fc:frame:image:aspect_ratio")
      frame.imageAspectRatio = meta.content;
  ...
  const buttonsar: FrameButton[] = Object.entries(buttons).reduce(
    (acc, [k, v]) => {
